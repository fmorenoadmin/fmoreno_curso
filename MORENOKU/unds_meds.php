<?php
	 ...
	 */
	class unds_meds extends database
	{
		private $table = 'unds_meds';
		private $tid = "id_um";//el PK de tipo_producto
		//--------------------------------
		function cbo($c1){//cbo es la abreviatura de combobox o select
			...
			//--------------------------------
			$inf .= '<option value="'.base64_encode(0).'">Seleccione la Unidad de Medida:</option>';//opcion html apra el selct
			$sql = "SELECT ".$this->tid.", nombres_um FROM ".$this->table." WHERE status = 1;";// siempre es este orden de secuencia sql y res
			...
			if ($res){// se esta validad si $res contiene informacion o no
				if ($res->num_rows > 0) {//si el numerod e filas del resultado es mayor que 0 
					while($row = mysqli_fetch_array($res)){//voy a recorrer fila por fila el resultado como un arreglo
						$inf .= '<option value="'.base64_encode($row[$this->tid]).'">'.$row['nombres_um'].'</option>';//estoy agregando mas ocpiones de aceurdo a la fila con la informacion que estoy trayendod e la base de datos, 
					}
					$data->result = 1;
